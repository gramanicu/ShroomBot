  ...
  public async execute(
    ...
    if (regServer) {
      if (LolApi.checkApiLimit()) {
        ...
        axios
          ...
          .then((res) => {
            ...
            const embed = new Discord.MessageEmbed()
              .setColor("#0099ff")
              .setTitle(
                `Here is some info about __**${res.data["name"]}**__ (${args[
                  args.length - 1
                ].toUpperCase()})`
              )
            ...
            axios
              ...
              .then((res) => {
                const champNum = 5;
                const champId: number[] = res.data
                  .slice(0, champNum)
                  .map((data: any) => parseInt(data["championId"]));
                const champMastery: number[] = res.data
                  .slice(0, champNum)
                  .map((data: any) => parseInt(data["championPoints"]));
                const champLevel: number[] = res.data
                  .slice(0, champNum)
                  .map((data: any) => parseInt(data["championLevel"]));

                const champName: string[] = champId.map((id) =>
                  LolApi.getChampName(id)
                );
                ...
                axios
                  ...
                  .then((res) => {
                    ...
                    champs.forEach((champion) => {
                      embed.addField(
                        `${champion.name}`,
                        `${Math.floor(
                          champion.mastery / 1000
                        )} k mastery points, Level ${champion.level}`,
                        false
                      );
                    });
                    embed.setTimestamp();
                  ...
                  .catch(() => {
                    process.stderr.write(
                      `Couldn't find ranked data for ${summonerName}`
                    );
        ...
        process.stderr.write("Exceeded rate limits\n");
        msgObject.channel.send(
          "Teemo ran out of shrooms! Try again in a minute."
        );
      }
    } else {
