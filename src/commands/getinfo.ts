                    ...
                    axios
                        ...
                        .then((res) => {
                            ...
                            const champId: number[] = res.data
                                .slice(0, champNum)
                                .map((data: any) =>
                                    parseInt(data['championId'])
                                );
                            const champMastery: number[] = res.data
                                .slice(0, champNum)
                                .map((data: any) =>
                                    parseInt(data['championPoints'])
                                );
                            const champLevel: number[] = res.data
                                .slice(0, champNum)
                                .map((data: any) =>
                                    parseInt(data['championLevel'])
                                );
                            ...
                            axios
                                .get(route3)
                                .then((res) => {
                                    const soloqData = res.data.filter(
                                        (queue: any) => {
                                            return (
                                                queue['queueType'] ===
                                                'RANKED_SOLO_5x5'
                                    ...
                                    if (soloqData) {
                                        const tier = soloqData[
                                            'tier'
                                        ].toLowerCase();
                                        const rank = soloqData[
                                            'rank'
                                        ].toUpperCase();
